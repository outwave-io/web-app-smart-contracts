...
contract('Organization Event Manager', () => {
  describe('withdraw native tokens / behavior ', () => {
    ...
    it('should allow user (user2) to actually purchase in native token', async () => {
      ...
      assert.isBelow(
        parseInt(await web3.eth.getBalance(lockAddress)),
        parseInt(keyPrice)
      )
    })
    ...
    it('shuold allow event owner withdraw', async () => {
      const initialBalance = ethers.BigNumber.from(
        await web3.eth.getBalance(eventOwner.address)
      )
      await outwave
        .connect(eventOwner)
        .withdraw(lockAddress, await web3.eth.getBalance(lockAddress)) // eth, withdraw al
      const afterWithDrawBalance = ethers.BigNumber.from(
        await web3.eth.getBalance(eventOwner.address)
      )
  ...
  describe('withdraw ERC20 tokens / behavior ', () => {
    ...
    before(async () => {
      ...
      ;[tokenDai] = await require('../helpers/deploy').deployErc20Tokens(
        owner,
        proxyOwner
      )
      ...
      let balance = await tokenDai.balanceOf(user2.address)
    ...
    it('should allow user (user2) to actually purchase in ERC20 token', async () => {
      ...
      assert.isBelow(
        parseInt(await tokenDai.balanceOf(lockAddress)),
        parseInt(keyPrice)
      )
    })
    ...
    it('shuold allow event owner withdraw and should have balance', async () => {
      ...
      await outwave
        ...
        .withdraw(
          lockAddress,
          (await tokenDai.balanceOf(lockAddress)).toString()
        )
  ...
  describe('withdraw  / security ', () => {
    ...
    it('shuold NOT allow withdraw to user that is not owner of lock (user3)', async () => {
      await reverts(
        outwave.connect(user3).withdraw(lockAddress, 1), // erc20, withdraw al
        'USER_NOT_LOCK_OWNER'
      )
