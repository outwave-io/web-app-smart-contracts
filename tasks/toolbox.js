...
task('tool:createEvent', 'create Event and returns lock address')
  ...
  .setAction(async ({
    ...
    const txEv = await outwave
      .eventCreate(
        web3.utils.padLeft(web3.utils.asciiToHex(eventid), 64),
        lockname,
        keytokenaddr,
        keyprice,
        keyquantity,
...
task('tool:call:keyburner', 'mint some keys and burn them ??')
...
task('tool:call:purchasekeyfor', 'buy a key for the specified address')
  //.addParam('keyburnaddr', 'the key burner address')
  .addParam('lockaddr', 'the public lock address')
  .addParam('recipientaddr', 'the recipient that will receive the key')
  .setAction(async ({ lockaddr, recipientaddr }, { ethers }) => {
